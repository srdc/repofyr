...
import io.onfhir.config.{FhirPaginationMode, FhirResultDefaults, FhirSearchTotalHandling, FhirServerConfig}
...
/**
  ...
  */
class FHIRResultParameterResolver(fhirConfig:FhirServerConfig, defaults: FhirResultDefaults) {
  /**
  ...
  def resolveCountPageParameters(resultParameters:List[Parameter]):(Int, Either[Int, (Seq[String], Boolean)]) = {
    ...
    defaults.paginationMode match {
      case FhirPaginationMode.Page =>
        offset match {
          case Some(o) => count -> Right(o)
          case None => count -> Left(page)
        }
      case FhirPaginationMode.Offset =>
  ...
  def resolveTotalParameter(resultParameters:List[Parameter]):Boolean = {
    resultParameters
      ...
      .getOrElse(defaults.totalHandling.code) != FhirSearchTotalHandling.None.code
  }
