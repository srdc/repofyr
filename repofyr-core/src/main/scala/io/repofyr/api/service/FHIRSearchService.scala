...
class FHIRSearchService(transactionSession: Option[TransactionSession] = None) extends FHIRInteractionService(transactionSession)  {
  ...
  private def constructBundle(
    ...
    val pagingLinks =
      if(OnfhirConfig.fhirResultDefaults.paginationMode == FhirPaginationMode.Offset || parameters.exists(p => p.name == FHIR_SEARCH_RESULT_PARAMETERS.SEARCH_AFTER || p.name == FHIR_SEARCH_RESULT_PARAMETERS.SEARCH_BEFORE))
