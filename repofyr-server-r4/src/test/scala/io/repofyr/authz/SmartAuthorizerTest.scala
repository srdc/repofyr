...
import io.onfhir.config.{FSConfigReader, FhirSearchHandling}
...
import io.onfhir.authz.AuthzConstraints
...
class SmartAuthorizerTest extends Specification {
  ...
  "SmartAuthorizer" should {
    ...
    "authorize or reject for patient level scopes for patient resource compartment=resourcetype" in {
      ...
      val resourceChecker = new io.repofyr.api.util.ResourceChecker(serverConfig, OnfhirConfig.fhirEndpointSettings)
