...
class FHIRPatchEndpointTest extends OnFhirTest with FHIREndpoint {
  ...
  "FHIR Patch Endpoint" should {
    "handle JSON patch for a resource" in {
      Put("/" + OnfhirConfig.serverSettings.baseUri + "/" + resourceType + "/" + observationId, HttpEntity(obsBP)) ~> fhirRoute ~> check {
      ...
      Patch("/" + OnfhirConfig.serverSettings.baseUri + "/" + resourceType + "/" + observationId, HttpEntity.apply(jsonPatchContentType, jsonPatch)) ~> fhirRoute ~> check {
      ...
      Get("/" + OnfhirConfig.serverSettings.baseUri + "/" + resourceType + "/" + observationId)~> fhirRoute ~> check {
        eventually(status === StatusCodes.OK)
        val result = responseAs[Resource]
        FhirPathEvaluator().evaluateString("component[0].code.coding[0].code", result) mustEqual Seq("test")
        FhirPathEvaluator().evaluateString("component[0].code.coding[1].code", result) mustEqual Seq("test2")
    ...
    "handle FHIR Path patch for a resource" in {
      ...
      Patch("/" + OnfhirConfig.serverSettings.baseUri + "/" + resourceType + "/" + observationId, HttpEntity.apply(jsonFhirContentType, fhirPathPatch)) ~> fhirRoute ~> check {
        eventually(status === StatusCodes.OK)
        val result = responseAs[Resource]
        FhirPathEvaluator().evaluateString("component[0].code.coding[0].code", result) mustEqual Seq("test")
        FhirPathEvaluator().evaluateString("component[0].code.coding[1].code", result) mustEqual Seq("test2")
    ...
    "reject JSON patch for a resource if patch is not valid" in {
      ...
      Patch("/" + OnfhirConfig.serverSettings.baseUri + "/" + resourceType + "/" + observationId, HttpEntity.apply(jsonPatchContentType, jsonPatchProblem1)) ~> fhirRoute ~> check {
        eventually(status === StatusCodes.PreconditionFailed)
      }
      Patch("/" + OnfhirConfig.serverSettings.baseUri + "/" + resourceType + "/" + observationId, HttpEntity.apply(jsonPatchContentType, jsonPatchProblem2)) ~> fhirRoute ~> check {
        eventually(status === StatusCodes.BadRequest)
      }
      Patch("/" + OnfhirConfig.serverSettings.baseUri + "/" + resourceType + "/" + observationId, HttpEntity.apply(jsonPatchContentType, jsonPatchProblem3)) ~> fhirRoute ~> check {
        eventually(status === StatusCodes.BadRequest)
      }
    }

    "reject FHIR Path patch for a resource if patch is not valid" in {
      Patch("/" + OnfhirConfig.serverSettings.baseUri + "/" + resourceType + "/" + observationId, HttpEntity.apply(jsonFhirContentType, fhirPathPatchProblem1)) ~> fhirRoute ~> check {
        eventually(status === StatusCodes.BadRequest)
      }
      Patch("/" + OnfhirConfig.serverSettings.baseUri + "/" + resourceType + "/" + observationId, HttpEntity.apply(jsonFhirContentType, fhirPathPatchProblem2)) ~> fhirRoute ~> check {
        eventually(status === StatusCodes.BadRequest)
      }
    }
    ...
    "handle conditional JSON patch for a resource" in {
      ...
      Patch("/" + OnfhirConfig.serverSettings.baseUri + "/" + resourceType + "?code=85354-9", HttpEntity.apply(jsonPatchContentType, jsonPatch)) ~> fhirRoute ~> check {
        eventually(status === StatusCodes.OK)
        val result = responseAs[Resource]
        FhirPathEvaluator().evaluateString("component[0].code.coding[0].code", result) mustEqual Seq("test")
        FhirPathEvaluator().evaluateString("component[0].code.coding[1].code", result) mustEqual Seq("test2")
      ...
      Patch("/" + OnfhirConfig.serverSettings.baseUri + "/" + resourceType  + "?code=85354-0", HttpEntity.apply(jsonPatchContentType, jsonPatch)) ~> fhirRoute ~> check {
    ...
    "handle fhir patch with expression - Goal status updating example" in {
      Post("/" + OnfhirConfig.serverSettings.baseUri + "/" + "Goal", HttpEntity(bodyWeightGoal)) ~> fhirRoute ~> check {
        eventually(status === StatusCodes.Created)
      }
      ...
      Patch("/" + OnfhirConfig.serverSettings.baseUri + "/" + "Goal"  + conditionalQuery, HttpEntity.apply(jsonFhirContentType, goalStatusPatch)) ~> fhirRoute ~> check {
        eventually(status === StatusCodes.OK)
        val result = responseAs[Resource]
      ...
      Patch("/" + OnfhirConfig.serverSettings.baseUri + "/" + "Goal"  + conditionalQuery, HttpEntity.apply(jsonFhirContentType, goalStatusPatch2)) ~> fhirRoute ~> check {
        eventually(status === StatusCodes.OK)
        val result = responseAs[Resource]
